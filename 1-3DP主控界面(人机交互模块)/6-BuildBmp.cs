...
namespace BuildBMP
{
    class Build1BitBmp//单色
    {
        ...
        /// <summary>
        /// 20200506新建批注：修正CLI模型中每条LineList的位置
        /// </summary>
        /// <param name="cli"></param>
...
#if false//是否清楚CLI的位置信息：是否清除绝对位置信息：20200514
            cli.Dimension[0].x += X;
            cli.Dimension[1].x += X;
            cli.Dimension[0].y += Y;
            cli.Dimension[1].y += Y;
...
#endif
            //CLI的所有坐标值修改为相对值：20200514
            for (int i = 0; i < cli.LayerNumber; i++)//遍历所有层：20200506新建批注
            {
                for (int j = 0; j < cli.LayerLine[i].LineList.Count; j++)//遍历所有LineList：20200506新建批注
                {
                    LineChange.TranslationDirect(cli.LayerLine[i].LineList[j], X, Y);//所有点坐标复位为0:20200514
                }
            }
        }    
