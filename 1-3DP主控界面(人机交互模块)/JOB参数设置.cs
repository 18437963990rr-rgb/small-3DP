...
namespace BinderJetting
{
    ...
    public class RYSYSParam : INotifyPropertyChanged, ICloneable//C#中，通知类的属性值已经更改，可以避免大量的通用事件的使用；其中关键是属性的理解及和lambda表达式的使用方法
    {
        ...
        public double m_dYJetOff = 0/*2026-05-07：为切换纯 Meteor 软件补偿临时统一置0；修改前默认值=15mm*//*20*/;//Y向起打位置修订:20210311新增//20210312修正：依据实际测量的成型缸体截面尺寸，进行为修改//20230319新建：此值修改为默认值0，消除此前的相关BUGS       m_nXPrintDpiIndex
