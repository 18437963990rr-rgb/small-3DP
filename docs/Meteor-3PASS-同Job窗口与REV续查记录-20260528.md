...
## 2026-05-29 09:45 补充：batch 改为 STARTSCAN 绑 pass gate

09:24 测试证明，旧 batch 会在 Pass0 扫程开始后 1 秒内连续发完 Pass1/Pass2 的 `STARTSCAN+IMAGE+ENDDOC`。日志中 Pass1 `STARTSCAN REV` 发生在真实 Pass1 运动前约 6 秒，Pass2 `STARTSCAN FWD` 发生在真实 Pass2 运动前约 12 秒；这解释了“只有 Pass0 喷墨”。

已调整为新的默认 batch 行为：

- `METEOR_BATCH_SWATH_MODE=1` 仍表示启用 batch/预处理思路。
- 默认 `METEOR_BATCH_STARTSCAN_GATE_SPLIT=1`：先把 3 个 strip 切好并缓存到软件内存，但每个 pass 的 `STARTSCAN+IMAGE+ENDDOC` 仍等待对应 `Pass0/Pass1/Pass2` gate 后才发送。
- `WaitPassSwathMeteorReady()` 在该模式下不再跳过等待；运动线程会等该 pass swath 进入 PCC 后再启动 X 扫程。
- 如需回到旧的连续早发 STARTSCAN 试验，可显式设置 `METEOR_BATCH_STARTSCAN_GATE_SPLIT=0`，但不建议联机使用。

下一轮推荐环境：

- `METEOR_BATCH_SWATH_MODE=1`
- `METEOR_BATCH_STARTSCAN_GATE_SPLIT=1` 或不设置
- `METEOR_BATCH_ENDJOB_IMMEDIATE=0`
