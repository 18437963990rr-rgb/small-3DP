    ...
    public static class MeteorPrintEngine
    {
        ...
        public static bool TryCompleteDeferredEndJob(string stage)
        {
            ...
            if (endJobRet)
            {
                ...
                bool idleReady = WaitForPccReadyForNewStartJob(Math.Min(idleTimeoutMs, 5000), stage + ":AfterEndJob");
                if (!idleReady)
                {
                    lock (SyncRoot)
                    {
                        idleReady = TryForcePccIdleForNextStartJobUnlocked(stage + ":PostDeferredEndJob", idleTimeoutMs, allowPiSetHome: false);
                    }
                }
        ...
        {
            return (bmStatusBits & BM_SCANNING) == 0;
        }
        ...
        public static bool SendStartJob(int reserved, uint imageWidth)
        {
            ...
            lock (SyncRoot)
            {
                ...
                for (int attempt = 0; attempt <= retryCount; attempt++)
                {
                    ...
                    Log4Net.Info($"Meteor SendStartJob: PCMD_STARTJOB busy(r=14), retry {attempt + 1}/{retryCount} after {retryDelayMs}ms; JobId={jobId} imageWidth={effectiveWidth}");
                    if (retryDelayMs > 0)
        ...
        public static int WriteImageLayer(ref royal.LPPRTIMG_LAYER layer, IntPtr imgPtr, int bytes)
        {
            ...
            lock (SyncRoot)
            {
                ...
                if (imageMaxWidthPx > 0 && printWidthForRevPx > imageMaxWidthPx)
                    printWidthForRevPx = imageMaxWidthPx;
                ...
                if (imageMaxWidthPx > 0 && printWidthPx > imageMaxWidthPx)
                {
                    Log4Net.Info($"[MeteorXStart] ImageMaxWidthApplied maxWidthPx={imageMaxWidthPx} originalWidthPx={actualWidth} passIndex={pendingPass} utc={DateTime.UtcNow:O}");
                    printWidthPx = imageMaxWidthPx;
                }
