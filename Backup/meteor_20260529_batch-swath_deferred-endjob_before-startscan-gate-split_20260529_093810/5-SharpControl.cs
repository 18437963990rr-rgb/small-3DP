        ...
        public void RenderToWic(bool action, int index, int subindex, int RePrintTimes, int ActualStartNum)//subindex:重喷索引，取值为0-1-2-3-....-n//201030新增：//20230317修改：修复中断打印之后，重新启动设置新区间，打印过程中的实际传输实际仍然按照第1层数据发送的BUG
        {
            if (action == true)
            {
                ...
                if (SimplifiedMeteorAutoFlowMode)
                {
                    ...
                    if (batchSwathMode)
                        Log4Net.Info("RenderToWic: SimplifiedMeteorAutoFlowMode BATCH 模式 — Pass0 门控后连续发送 3 strip；EndJob 延后至 Pass2 扫程结束");
                    else
                    ...
                    if (anySwathSent)
                    {
                        if (batchSwathMode && MeteorPrintEngine.IsBatchEndJobImmediateEnabled())
                        {
                            MeteorPrintEngine.MarkDeferredEndJobAfterPassSwaths("RenderToWic:SimplifiedMeteorAutoFlowMode:BatchImmediateEndJobSuppressedForMotionSafety");
                            Log4Net.Info($"RenderToWic: SimplifiedMeteorAutoFlowMode HiPrint-style batch 已连续发送3PASS swath，但当前3PASS本地运动仍在执行，立即EndJob不安全，已强制改为延后EndJob，stripCount={stripIndexSimple}");
                        }
                        else
                        {
