...
namespace BinderJetting
{
    public partial class 手动操作 : Form
    {
        ...
        //                  电机“正反”对调后，若点动/定位/回零是同向一起反了，将此量在 +1 与 -1 之间切换。
        // InkCarXDisplaySign：仅影响 GetCurrentPos(1)、界面/日志里“显示毫米”与 ToDisplayMm(1,…)。
        //                  若编码器计数方向随电机变了、画面坐标与真机左右对调，再改此项（与命令脉冲独立）。
        // X 回零“寻哪一端限位”：button18 → RunInkCarAxisHome(1, SearchPositiveDirection, …)。true=轮询正限位 FlagPosLimit1；false=负限位 FlagNegLimit1。
        //                  限位线对调后若仍用原逻辑，会出现找错限位或超时；需与接线一致后改 true/false，并核对回零分支里 rollbackTargetMm / homeEncPos。
        // 注意：XMoveBtn case3/4（点动）当前 TrapMotion 未乘 InkCarXCmdSign；若只改了常数而点动仍反，请给点动脉冲补上 * InkCarXCmdSign，或与 case3/4 的正负对调一致。
        // X：命令层取反（Trap/JOG/回零 一致）；显示层符号仅用于监视窗与 GetCurrentPos 一致（与硬件脉冲方向独立）
        // Y：仅依赖 GoogolMotionMap.GetEncPos 对轴2 的取反；勿与 X 镜像混用额外显示取反，否则 BackToStation 位移会与界面坐标不一致
        ...
        private const double InkCarPassPitchYMm = PrintRasterConfig.MeteorPassPitchYMm;
...
/// <summary>Pass0 打印前 X 准备点（mm），默认 485；与 Meteor <see cref="MeteorPrintEngine.GetInkCarScanApproachHighEndMm"/> 一致。</summary>
        ...
        /// <summary>X 扫描高端 / 反向扫终点 / 层末收口（mm），默认 485。</summary>
        static double InkCarScanHighXMm => MeteorPrintEngine.GetInkCarScanHighEndMm();
        /// <summary>X 扫描低端 / 正向扫终点（mm），默认 15。</summary>
        static double InkCarScanLowXMm => MeteorPrintEngine.GetInkCarScanLowEndMm();
        ...
        // PowderCarHomeSearchSign：仅 SetBackHome 第一段 search_home（去找 CAPTURE_HOME）的符号，与 PowderCarCmdSign 分开：
        //   若点动/计数器已正确，但按 PowderCarHomeBtn 校准却「先朝负向」搜 Home，是因为曾把 search 也乘了 PowderCarCmdSign。
        //   寻 Home 应先朝能触发 Home 开关的一侧跑：该侧在规划坐标里为正则置 +1，为负则置 -1（与 1000mm 搜索行程相乘）。
        // PowderCarHomeRetractMm：SetBackHome 第二段，targetPulse = 捕获脉冲 + (本值×1000)。即原固高例程里「捕获后的增量 mm」（有符号）。
        //   曾用 -116 表示相对捕获点再移 -116mm（回退约 116mm）；与 m_dPowderCarHomeposition（回零后写入的逻辑坐标）无关。
        //   若实测需在 116mm 基础上少退 15mm：-116+15 = -101。
        // PowderCarProcessSign：仅用于 BackToStation2（自动进给铺粉流程）的位移符号；默认 +1 与 AimPos-CurrentPos 同向。
        // PowderCarDisplaySign：仅 GetCurrentPos(7)；命令方向对但数显反了再改。
        // 限位语义：PowderCarHomeResetThread 开头 0x20/0x40 仍按固高「正/负限位」位；若仅调换限位接线未改脉冲，可能需对调判断或改 captures。
        // 其它工程：JOB调度「单层铺粉模块」里 GoHome(7,…) 若自动铺粉方向反了，需改该处 Ends 或本库与之一致的符号策略。
        const int PowderCarCmdSign = 1;
        const int PowderCarProcessSign = 1;
        const int PowderCarHomeSearchSign = 1;
        const double PowderCarHomeRetractMm = -106.0;//-116+10：在原先 -116mm 回退量上缩短 10mm
        const int PowderCarDisplaySign = 1;
        ...
        /// <summary>去程终点：触发成型缸让位下降后短延时即启动铺粉车回程（轴8异步到位，回程途中完成1.5mm下降）。</summary>
        ...
        private void InitializeSafetyCoordinatesSnapshotControls()
        {
            ...
            _safetyCoordinatesSnapshotLabel.BringToFront();
            _recordSafetyCoordinatesButton.BringToFront();
        }
        ...
        {
            Log4Net.Info("[CollisionSafetySnapshot] record button click received");
            try
            {
...
//#endif
                ...
                case 13://20200903批注:轴数由6轴提升到8轴（按钮文案：后退）
                    if (m_bMoveModeFlag[6] == false)
                    { JogMoveDown(7, m_bMoveModeFlag[6], m_sVel[6]); }
                    else {/*不执行任何操作*/}
                    break;
                case 14://20200903批注:轴数由6轴提升到8轴（按钮文案：前进）
                    if (m_bMoveModeFlag[6] == false)
...
// 2026-02-02修改：墨车轴切换到固高控制（4轴卡测试），注释Royal设备初始化
        ...
        protected override void OnFormClosed(FormClosedEventArgs e)
        {
            ...
            try
            {
                ...
                // EXO3 为步进电源，关闭窗体时勿 SetDo(3,false)，避免步进/24V 母线掉电
            }
            catch (Exception ex)
            {
                ...
                case 5://20220506批注：Y轴主运动-前进
                    ...
                    if (true/*!RoyalMap.m_bYSyncCtl*/)//同步运动标志位//20200421修改：100000对应是500mm;我修改为200000
                    {
                        ...
                        nSpeed = MM_TO_DOT(m_szMovSpeed, EncoderLinePerInch, 1);//20220506调整：必须调整成正确的运转速度
                        {
                            ...
                            int y1Position = (750000 / 1000) * InkCarYCmdSign;
                            double y1Vel = nSpeed;
        ...
        private void PowderCarHomeResetThread()//20220521新建：铺粉车校准，20260416修改为轴7
        {
            ...
            else//铺粉车位于正常停靠区
            {
                ...
                if (ReturnCode == true)//校准成功
                {
                    ...
                    Log4Net.Info($"PowderCarHomeResetThread: SetBackHome 后 BackToStation2, cur={cur:F3}mm, cfgHome={cfgHome:F3}mm, AimPos={AimPos:F3}mm, TrapSpace≈{trapMm:F3}mm, MovSpeed={MovSpeed}mm/s");
                    if (Math.Abs(trapMm) > 0.05)
                    {
                        BackToStation2(AimPos, MovSpeed, true);//20220520新建：单位为MM//此处：true为的等停，false为不等停//此处为不等停
                    }
                    else
                    {
                        Log4Net.Info("PowderCarHomeResetThread: |TrapSpace|≤0.05mm，跳过 BackToStation2");
                    }
                }
            }
        ...
        private bool IsPowderCarAtStationForFeed(out string reason)
        {
            double cur = GetCurrentPos(7);
            ...
            if (Math.Abs(cur - stationPos) <= PowderStationFeedPosToleranceMm
                ...
                || Math.Abs(cur - cfgHome) <= PowderStationFeedPosToleranceMm
                || Math.Abs(cur + cfgHome) <= PowderStationFeedPosToleranceMm)
            {
                reason = $"粉车在落粉站附近 cur={cur:F1}mm";
                return true;
            }

            reason = $"粉车不在落粉站，拒绝接粉：cur={cur:F1}mm，expected≈{stationPos:F1}/{stationNeg:F1}mm";
            return false;
        }
        ...
        private bool StartPowderStationFeedMotion(string sourceTag, bool waitStop)
        {
            ...
            {
                Log4Net.Info($"[{sourceTag}] skip station feed: disabled");
                return false;
            }

            string stationReason;
            if (!IsPowderCarAtStationForFeed(out stationReason))
            {
                Log4Net.Info($"[{sourceTag}] skip station feed: {stationReason}");
                return false;
            }
        ...
        private void WaitGoogolAxisTrapDone(short axis, string sourceTag, int timeoutMs)
        {
            ...
            {
                motionMap.GetAxisStatus(axis, out status);
                if ((status & 0x400) == 0)
                {
                    Thread.Sleep(30);
                    motionMap.GetAxisStatus(axis, out status);
            ...
            Log4Net.Info($"[{sourceTag}] axis{axis} trap wait timeout, sts=0x{status:X}");
        }

        private void FinishPowderStationFeedMotion(string sourceTag, bool feedStarted)
        {
            if (!feedStarted)
                return;

            WaitGoogolAxisTrapDone(PowderStationFeedAxis, sourceTag + "_axis3", 15000);
            motionMap.StopMotion(PowderStationFeedAxis);
            if (PowderStationFeedSettleMs > 0)
                Thread.Sleep(PowderStationFeedSettleMs);
            Log4Net.Info($"[{sourceTag}] station feed DONE");
        }

        private void WaitAxis8StableAfterTrap(string sourceTag)
        {
            DateTime axis8Deadline = DateTime.UtcNow.AddSeconds(3);
            double axis8Prev = GetCurrentPos(8);
            int axis8StableCount = 0;
            int axis8Status = 0;
            while (DateTime.UtcNow < axis8Deadline)
            {
                Thread.Sleep(20);
                double axis8Cur = GetCurrentPos(8);
                motionMap.GetAxisStatus(8, out axis8Status);
                axis8StableCount = (Math.Abs(axis8Cur - axis8Prev) <= 0.005 && (axis8Status & 0x400) == 0) ? (axis8StableCount + 1) : 0;
                axis8Prev = axis8Cur;
                if (axis8StableCount >= 3)
                    break;
            }
            Log4Net.Info($"[{sourceTag}] axis8 stable wait done, stableCount={axis8StableCount}");
        }

        /// <summary>铺粉去程终点：轴8 Trap 已下发（不等停），短 lead 后即回程；避免 WaitAxis8Stable+dwell 空等 2~3s。</summary>
        private void LeadPowderReturnAfterAxis8Trap(string sourceTag, int leadMs = PowderCarTravelEndAxis8LeadMs)
        {
            if (leadMs < 0)
                leadMs = 0;
            Log4Net.Info($"[{sourceTag}] axis8 trap async; leadMs={leadMs} before powder car return utc={DateTime.UtcNow:O}");
            if (leadMs > 0)
                Thread.Sleep(leadMs);
        }
...
#endif
            Log4Net.Info($"Axis8StepDiag: {sourceTag} parallel prep down done, before={axis8Before:F3}mm, after={GetCurrentPos(8):F3}mm, delta={GetCurrentPos(8) - axis8Before:F3}mm");
        }
        ...
        /// <summary>手动/调试：仅 DAC11 功率；EXO3 为步进电源，由电气面板手动保持，软件不自动拉低。</summary>
        private void OpenIRLamp(bool EnableLight)
        {
            if (EnableLight)
            {
                ...
                Log4Net.Info($"OpenIRLamp ON: IRPowerPercentage={pct:F1}% (DAC11 only, EXO{IrModuleEnableDo} manual)");
            }
            else
            {
                motionMap.TurnOffIrModule();
                Log4Net.Info($"OpenIRLamp OFF: DAC11 0V (EXO{IrModuleEnableDo} stepper/IR supply unchanged)");
            }
        }

        /// <summary>同步铺粉去程 IR：仅 DAC11 模拟量；EXO 使能由操作员手动控制，平时保持 0V。</summary>
        private void SetSyncCureIrDacPower(bool enablePresetPower)
        {
            if (enablePresetPower)
            {
                double pct = k_RYSYSParamAutoPrintParamInTest.m_dIRPowerPercentage;
                motionMap.SetIrModulePowerPercent(pct);
                Log4Net.Info($"SyncCure: IrDacPreset {pct:F1}% (DAC11 only, EXO{IrModuleEnableDo} manual)");
            }
            else
            {
                motionMap.TurnOffIrModule();
                Log4Net.Info($"SyncCure: IrDacOff 0V (DAC11 only, EXO{IrModuleEnableDo} manual)");
            }
        }

        private void EnsureSyncCureIrDacReleased(ref bool openIssued, ref bool closeIssued, string reason)
        {
            if (UVIRLightFlag || !openIssued || closeIssued)
                return;
            SetSyncCureIrDacPower(false);
            closeIssued = true;
            Log4Net.Info($"SyncCure: IrDacForceOff reason={reason}");
        }
        ...
        private SyncCureWindow ResolveSyncCureWindow()
        {
            ...
            double openPos = p.SyncCureOpenPosMm;
            double closePos = p.SyncCureClosePosMm;
        ...
        private static double GetSyncCureForwardMonitorEndMm(SyncCureWindow window)
        {
            return Math.Max(POWDERCAR_DROP_END, window.ClosePosMm);
        }
        ...
        **********************************************/
        ...
        /// <summary>自动清洗（非手动）累计次数，用于周期性物理寻零。</summary>
        ...
        private void InitializeDualRasterSamplingControls()
        {
            ...
            group.Visible = false; // Temporarily hide the dual-raster test panel to reserve this area for collision diagnostics.
        ...
        private bool WaitInkCarXAxisReach(double targetMm, double toleranceMm = 1.0, int timeoutMs = 12000)
        {
            ...
            while (true)
            {
                ...
                {
                    Log4Net.Info($"WaitInkCarXAxisReach: waiting, targetMm={targetMm:F3}, currentMm={currentMm:F3}, deltaMm={deltaMm:F3}, elapsedMs={(DateTime.Now - waitStart).TotalMilliseconds:F0}");
                    lastHeartbeat = DateTime.Now;
                }
        ...
        /// <summary>PiSetHome 精扫分支：数据 OFFICIAL-QUEUED 已预灌，运动跳过 Reach 门控以节约扫程时间。</summary>
        private bool IsInkCarFastOfficialQueuedMotionEnabled()
        {
            return IsMeteorPrecisionPiSetHomeMotionBranch();
        }

        private static void InvalidateInkCarMotionTimingCache()
        {
            _inkCarConservativeTimingCached = null;
        }
        ...
        private bool ReturnInkCarToCleanWaitStationAfterPass2(float movSpeed, double toleranceMm = 1.0, int timeoutMs = 120000, bool xReturnAlreadyStarted = false)
        {
            ...
            float correctionSpeed = Math.Min(movSpeed, 20.0f);
            const double settleDeltaMm = 0.02;
            const int settleWindowMs = 200;
            double previousX = double.NaN;
            double previousY = double.NaN;
            DateTime xLastMovedAt = waitStart;
            DateTime yLastMovedAt = waitStart;
            while ((DateTime.Now - waitStart).TotalMilliseconds < timeoutMs)
            {
                DateTime now = DateTime.Now;
                double x = GetCurrentPos(1);
                if (!double.IsNaN(previousX) && Math.Abs(x - previousX) > settleDeltaMm)
                    xLastMovedAt = now;
                previousX = x;
                ...
                if (!double.IsNaN(previousY) && Math.Abs(y - previousY) > settleDeltaMm)
                    yLastMovedAt = now;
                previousY = y;
                bool xNear = Math.Abs(x - INKCAR_CLEAN_STATION_X) <= toleranceMm;
                bool yNear = yStarted && Math.Abs(y - INKCAR_CLEAN_STATION_Y) <= toleranceMm;
                bool xProfileMoving = IsInkCarXProfileMoving();
                bool yProfileMoving = yStarted && IsInkCarAxisProfileMoving(2);
                bool xSettled = (now - xLastMovedAt).TotalMilliseconds >= settleWindowMs;
                bool ySettled = yStarted && (now - yLastMovedAt).TotalMilliseconds >= settleWindowMs;
                if (xNear && yNear && xSettled && ySettled && !xProfileMoving && !yProfileMoving)
                {
                    string reason;
                ...
                // TrapMotion 的异步高速回程可能在目标旁留约 1~3mm 余差；此前这里会一直等到总超时。
                // Profile 状态位会早于实际惯性结束，须同时确认编码器已连续静止后再补正。
                if (yStarted && correctionAttempts < maxCorrectionAttempts && xSettled && ySettled && !xProfileMoving && !yProfileMoving && (!xNear || !yNear))
                {
                    correctionAttempts++;
                    Log4Net.Info($"[InkCarTiming] WashXYOverlap residual correction attempt={correctionAttempts}/{maxCorrectionAttempts} x={x:F3} y={y:F3} targetX={INKCAR_CLEAN_STATION_X:F3} targetY={INKCAR_CLEAN_STATION_Y:F3} speed={correctionSpeed:F1} settleWindowMs={settleWindowMs}");
                    if (!xNear)
                        BackToStation((float)INKCAR_CLEAN_STATION_X, correctionSpeed, false, false, 1);
                    if (!yNear)
                        BackToStation((float)INKCAR_CLEAN_STATION_Y, correctionSpeed, true, false, 1);
                }

                Thread.Sleep(10);
            }

            Log4Net.Info($"[InkCarTiming] WashXYOverlap timeout targetX={INKCAR_CLEAN_STATION_X} targetY={INKCAR_CLEAN_STATION_Y} currentX={GetCurrentPos(1):F3} currentY={GetCurrentPos(2):F3} yStarted={yStarted} correctionAttempts={correctionAttempts}/{maxCorrectionAttempts} timeoutMs={timeoutMs}");
            return false;
        }
        ...
        private void LogInkCarLayerScan750ToPass2LowTiming(string stage)
        {
            ...
            Log4Net.Info($"[InkCarTiming] marker={stage} elapsedMs={elapsedMs:F0} targetMs=22000 conservativeTiming={IsInkCarConservativeTimingEnabled()} fastOfficialQueued={IsInkCarFastOfficialQueuedMotionEnabled()} utc={DateTime.UtcNow:O}");
        }
        ...
        private bool WaitInkCarYAxisReach(double targetMm, double toleranceMm = 1.0, int timeoutMs = 12000)
        {
            ...
            while (true)
            {
                ...
                double deltaMm = Math.Abs(currentMm - targetMm);
                if (deltaMm <= toleranceMm && !IsInkCarAxisProfileMoving(2))
                {
        ...
        public void EnsureInkCarAtCleanWaitStation(float movSpeed, string stage, double toleranceMm = 1.0, int timeoutMs = 120000)
        {
            ...
            bool needMoveY = Math.Abs(y0 - INKCAR_CLEAN_STATION_Y) > toleranceMm;
            if (needMoveX)
                BackToStation((float)INKCAR_CLEAN_STATION_X, movSpeed, false, false, 1);
            if (needMoveY)
                BackToStation((float)INKCAR_CLEAN_STATION_Y, movSpeed, true, false, 1);
                ...
                switch (MoveDirectionFlag)
                    {
                        ...
                        case true://Y 轴：固高编码器定位
                            if (AimPos >= 1 && AimPos <= (YMaxDistanceMM - 10))
                            {
                                ...
                                double currentMmYDisplay = ToDisplayMm(2, currentMmY);
                                ...
                                int moveYCmd = moveCountsY * InkCarYCmdSign;
                                Log4Net.Info($"BackToStation Y: before TrapMotion, moveCountsY={moveCountsY}, moveYCmd={moveYCmd} (InkCarYCmdSign={InkCarYCmdSign}), velCountPerMsY={velCountPerMsY:F3}, waitStop={WaitStopFLag}");
                                motionMap.TrapMotion(2, ref y1TrapPrm, moveYCmd, velCountPerMsY, 0, 0, WaitStopFLag);
...
#region 监控指令：铺粉拍摄位点1
                        ...
                        double syncCureMonitorEndMmManual = GetSyncCureForwardMonitorEndMm(syncCureWindowManual);
                        bool m_startLightFlag = false;
                        bool m_startLightFlag2 = false;
...
//#endif
                ...
                GoogolDigtalOut(14, false);
                        ...
                        double PosValue = 0;
                        int stage1Axis7StatusManual = 0;
                        do
                        {
                            PosValue = GetCurrentPos(7);
                            motionMap.GetAxisStatus(7, out stage1Axis7StatusManual);
                            TrySyncCureLightByPowderCarPos(PosValue, syncCureWindowManual, ref m_startLightFlag, ref m_startLightFlag2);
                            Thread.Sleep(10);
                        } while (PosValue < AimPos && (stage1Axis7StatusManual & 0x20) == 0 && (stage1Axis7StatusManual & 0x40) == 0);
                        ...
                        } while (PosValue <= syncCureMonitorEndMmManual);
                        ...
                        EnsureSyncCureIrDacReleased(ref m_startLightFlag, ref m_startLightFlag2, "NewAutoSupplyPowderThread_manual_afterForward");
...
#region 监控发送指令//20230113新建且批注：
        ...
        public void NewAutoSupplyPowderThread2CureFirst(ref SendMessageToCamera toCamera, int RecordLayerIndex, int RecordProcessIndex, float BackCleanMovSpeed, Action powderReturnSafeCallback = null)//20220512新建：新的上送粉铺粉逻辑
        {
            ...
            else//粉车位于正常停靠区
            {
                ...
                Log4Net.Info(msg);
                LeadPowderReturnAfterAxis8Trap("NewAutoSupplyPowderThread2CureFirst_beforeReturn");
...
#region 监控指令：铺粉拍摄位点1
                ...
                double syncCureMonitorEndMm = GetSyncCureForwardMonitorEndMm(syncCureWindow);
                bool m_startLightFlag = false;//开灯标志，确保开灯1次（去程全程共用，含112→445与445→1017）
                bool m_startLightFlag2 = false;//20220920新增：关灯标志，确保关灯1次
                Log4Net.Info($"NewAutoSupplyPowderThread2 SyncCureWindow: open={syncCureWindow.OpenPosMm:F1} close={syncCureWindow.ClosePosMm:F1} monitorEnd={syncCureMonitorEndMm:F1} IR={!UVIRLightFlag} power={k_RYSYSParamAutoPrintParamInTest.m_dIRPowerPercentage:F1}% IrDacOnly=EXO{IrModuleEnableDo}Manual");
                ...
                } while (PosValue <= syncCureMonitorEndMm);
                //while (PosValue <= 620) ;//20220512新建批注：有效区域宽度为360MM;起始打印位置：255MM;终止洒粉位置620MM
                ...
                EnsureSyncCureIrDacReleased(ref m_startLightFlag, ref m_startLightFlag2, "NewAutoSupplyPowderThread2_afterForward");
                ...
                msg = $"成形面高度下降指定厚度 TrapSpace{{{-TrapSpace}mm}}：TrapMoveUp(8, true, Convert.ToString(vel), Convert.ToString(TrapSpace), true, false）";
                Log4Net.Info(msg);
                LeadPowderReturnAfterAxis8Trap("NewAutoSupplyPowderThread2_beforeReturn");
...
#region 监控指令：喷墨拍摄位点1
                        ...
                        BackToStation((float)InkCarScanApproachXMm, (float)ReturnVelocity1, false, true, 1);
                        BackToStation((float)InkCarScanLowXMm, (float)ReturnVelocity1, false, true, 1);//停靠在右侧，向左侧运动打印幅面<---------------
...
#region 监控指令：喷墨拍摄位点2
                        ...
                        break;
                    case 1:
                        BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, true, 1);//停靠在右侧，向左侧运动打印幅面--------------->
...
#endregion
                        BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, true, 1);//末道后与历史第6 Pass 一致：X 收口到 425mm
                        break;
                    default:
                        break;
                }
...
#region 监控发送指令//20230113新建且批注：
        ...
        /// TODO::修改打印辐面等常数 
        /// <summary>
        /// [2026-06-01] 非当前 3PASS 量产主路径：经 EquipmentMotionLogic3 Command=4 调用（SinglePassPrintMode 等）。
        /// 当前 TwoPassPrintPerThreeTimes 量产墨车运动见 <see cref="AutoPrintThread5"/>（Command=6）。
        /// </summary>
...
#region 监控指令：喷墨拍摄位点1
                            ...
                            BackToStation((float)InkCarScanApproachXMm, (float)ReturnVelocity2/*ReturnVelocity1*/, false, true, 1);//停靠在右侧，向左侧运动打印幅面<---------------
...
#endregion
                            BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, true, 1);//与历史 case5 一致：先收 X 至 425
...
#region 监控指令：喷墨拍摄位点7
                                ...
                                Log4Net.Info("AutoPrintThread5: 已禁用自动打印路径在回清洗/待机位后的重新开启闪喷逻辑。");
                            }
...
#region 监控指令：喷墨拍摄位点1
                            ...
                            BackToStation((float)InkCarScanLowXMm, (float)ReturnVelocity1, false, true, 1);//打印//
...
#region 监控指令：喷墨拍摄位点2
                            ...
                            break;
                        case 1:
                            BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, true, 1);//打印一次：停靠在右侧，向左侧运动打印幅面--------------->
...
#region 监控指令：喷墨拍摄位点3
                            ...
                            break;
                        case 2://末道：打印后收口 + 回站
                            BackToStation((float)InkCarScanLowXMm, (float)ReturnVelocity1, false, true, 1);//打印一次
...
#region 监控指令：喷墨拍摄位点4
...
#endregion
                            BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, true, 1);//收口
                            ...
                            else
                            {
                                ...
                                bool nRetVal2 = MeteorPrintEngine.SetFlash(false);//打开闪喷
...
#region 监控指令：喷墨拍摄位点1
                        ...
                        BackToStation((float)InkCarScanLowXMm, (float)ReturnVelocity1, false, true, 1);
        ...
        /// <summary>
        /// [2026-06-01] 自动打印墨车运动量产主实现（3PASS+Meteor）。
        /// 调用链：工业控制.cs 打印层循环 → EquipmentMotionLogic3(Command=6) → 本方法。
        /// Meteor 数据链在 5-SharpControl.RenderToWic / MeteorPrintEngine，与本方法通过 ScanGate 对齐。
        /// </summary>
        ...
        {
            return EnvEquals("METEOR_AUTO_PRINT_MOTION_BRANCH", MeteorAutoPrintMotionBranchPhysicalFast);
        }

        private static void InvokePowderReturnSafeCallback(Action callback, string sourceTag)
        {
            if (callback == null)
                return;
            ...
            {
                Log4Net.Info($"[PowderReturnEarlyPrep] callback failed source={sourceTag}, ex={ex.Message}");
            }
        }

        /// <summary>
        /// 快速层间模式专用：粉车回程已越过安全点后，预热下一层 Pass0 的 STARTJOB 并让墨车驶向准备位。
        /// 正式扫描仍由原有打印主线程在准备位到达后发 Pass0 Ready，避免粉车线程直接改写扫描状态机。
        /// </summary>
        public bool BeginPowderReturnEarlyPass0Preparation(int nextLayerK, float backCleanSpeed, string sourceTag)
        {
            ...
            {
                Log4Net.Info($"[PowderReturnEarlyPrep] skipped source={sourceTag} layerK={nextLayerK}, reason={reason}");
                return false;
            }
        ...
        /// <summary>
        /// 试验性层间并行：Pass2 已到低端且墨车已开始回清洗位时，提前完成轴8、粉站送料和受硬门控的轴7第一阶段。
        /// 轴7先到 180mm，待墨车 X>=100mm 且 Y 位于实测区间后才继续到原第一阶段目标。
        /// </summary>
        private bool WaitPass2ParallelAxis7Reach(double targetMm, int timeoutMs)
        {
            DateTime deadline = DateTime.UtcNow.AddMilliseconds(timeoutMs);
            while (DateTime.UtcNow < deadline)
            {
                double currentMm = GetCurrentPos(7);
                int axisStatus = 0;
                motionMap.GetAxisStatus(7, out axisStatus);
                if ((axisStatus & 0x20) != 0 || (axisStatus & 0x40) != 0)
                {
                    Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 limit/alarm while targeting {targetMm:F3}mm; current={currentMm:F3} status=0x{axisStatus:X}");
                    return false;
                }
                if (Math.Abs(currentMm - targetMm) <= 2.0)
                    return true;
                Thread.Sleep(10);
            }

            Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 reach timeout target={targetMm:F3} current={GetCurrentPos(7):F3}");
            return false;
        }

        private bool WaitPass2ParallelInkXRelease(int timeoutMs)
        {
            DateTime deadline = DateTime.UtcNow.AddMilliseconds(timeoutMs);
            while (DateTime.UtcNow < deadline)
            {
                double inkX = GetCurrentPos(1);
                if (inkX >= Pass2ParallelInkXReleaseMm)
                    return true;
                Thread.Sleep(10);
            }

            Log4Net.Info($"[Pass2ParallelPowderPrep] ink X release timeout threshold={Pass2ParallelInkXReleaseMm:F3} current={GetCurrentPos(1):F3}");
            return false;
        }
        ...
        private bool RunPass2ParallelPowderAxis7ForwardStage(int completedLayerK)
        {
            ...
            double initialInkX = GetCurrentPos(1);
            double initialInkY = GetCurrentPos(2);
            if (targetMm <= 0 || moveSpeed <= 0)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 stage skipped completedLayerK={completedLayerK}; target={targetMm:F3} speed={moveSpeed:F3}");
                return false;
            }
            if (initialInkX < Pass2ParallelInkXReleaseMm && initialAxis7 > Pass2ParallelAxis7HoldMm + 2.0)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 stage rejected completedLayerK={completedLayerK}; inkX={initialInkX:F3} below release={Pass2ParallelInkXReleaseMm:F3}, axis7={initialAxis7:F3} exceeds hold={Pass2ParallelAxis7HoldMm:F3}");
                return false;
            }
            if (initialInkY < Pass2ParallelInkYSafeMinMm || initialInkY > Pass2ParallelInkYSafeMaxMm)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 stage rejected completedLayerK={completedLayerK}; inkY={initialInkY:F3} outside measuredRange=[{Pass2ParallelInkYSafeMinMm:F3},{Pass2ParallelInkYSafeMaxMm:F3}]");
                return false;
            }

            double rollerAxis6Speed = Math.Abs(k_RYSYSParamAutoPrintParamInTest.m_dPowderCarBackRollerSpeed);
            double rollerAxis5Speed = Math.Abs(k_RYSYSParamAutoPrintParamInTest.m_dPowderCarBackRollerAxis5Speed);
            bool rollerAxis6Positive = k_RYSYSParamAutoPrintParamInTest.m_nRollerRotateDirection == 0;
            bool rollerAxis5Positive = k_RYSYSParamAutoPrintParamInTest.m_nPowderCarBackRollerAxis5RotateDirection == 0;
            double rollerAxis6Turns = rollerAxis6Speed > 0 ? rollerAxis6Speed * Math.Max(0, targetMm - initialAxis7) / moveSpeed : 0;
            double rollerAxis5Turns = rollerAxis5Speed > 0 ? rollerAxis5Speed * Math.Max(0, targetMm - initialAxis7) / moveSpeed : 0;
            if (rollerAxis6Turns > 0.001)
            {
                if (rollerAxis6Positive)
                    TrapMoveUp(6, true, Convert.ToString(rollerAxis6Speed), Convert.ToString(rollerAxis6Turns), true, false);
                else
                    TrapMoveDown(6, true, Convert.ToString(rollerAxis6Speed), Convert.ToString(rollerAxis6Turns), true, false);
            }
            if (rollerAxis5Turns > 0.001)
            {
                if (rollerAxis5Positive)
                    TrapMoveUp(5, true, Convert.ToString(rollerAxis5Speed), Convert.ToString(rollerAxis5Turns), true, false);
                else
                    TrapMoveDown(5, true, Convert.ToString(rollerAxis5Speed), Convert.ToString(rollerAxis5Turns), true, false);
            }

            if (initialInkX < Pass2ParallelInkXReleaseMm)
            {
                double holdTargetMm = Math.Min(targetMm, Pass2ParallelAxis7HoldMm);
                BackToStation2(holdTargetMm, moveSpeed, false);
                if (!WaitPass2ParallelAxis7Reach(holdTargetMm, 10000)
                    || !WaitPass2ParallelInkXRelease(10000))
                    return false;
            }

            BackToStation2(targetMm, moveSpeed, false);
            if (!WaitPass2ParallelAxis7Reach(targetMm, 10000))
                return false;

            Log4Net.Info($"[Pass2ParallelPowderPrep] axis7 stage completed completedLayerK={completedLayerK}; initialInkX={initialInkX:F3} initialInkY={initialInkY:F3} initialAxis7={initialAxis7:F3} hold={Pass2ParallelAxis7HoldMm:F3} releaseInkX={Pass2ParallelInkXReleaseMm:F3} target={targetMm:F3}");
            return true;
        }

        private bool BeginPass2ParallelPowderPreparation(int completedLayerK)
        {
            if (!EnvEquals(PowderReturnEarlyPass0PrepModeEnv, "1") || !IsMeteorPhysicalHomeFastPrintMotionBranch())
                return false;
            if (completedLayerK <= 1)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] skipped completedLayerK={completedLayerK}; first layer remains sequential");
                return false;
            }
            if (k_RYSYSParamAutoPrintParamInTest.m_nRecoaterMode != 0)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] skipped completedLayerK={completedLayerK}; only recoaterMode=0 is validated for axis7 early stage");
                return false;
            }

            lock (Pass2ParallelPowderPrepSync)
            {
                if (_pass2ParallelPowderPrepThread != null && _pass2ParallelPowderPrepThread.IsAlive)
                {
                    Log4Net.Info($"[Pass2ParallelPowderPrep] skipped completedLayerK={completedLayerK}; previous prep is still running");
                    return false;
                }
                ...
                _pass2ParallelPowderPrepThread.IsBackground = true;
        ...
        private static bool TryConsumePass2ParallelPowderPreparation(int completedLayerK)
        {
            Thread pendingThread = null;
            lock (Pass2ParallelPowderPrepSync)
            {
                if (_pass2ParallelPowderPrepLayerK == completedLayerK)
                {
                    _pass2ParallelPowderPrepLayerK = int.MinValue;
                    return true;
                }
                if (_pass2ParallelPowderPrepThread != null && _pass2ParallelPowderPrepThread.IsAlive)
                    pendingThread = _pass2ParallelPowderPrepThread;
            }

            if (pendingThread != null)
            {
                Log4Net.Info($"[Pass2ParallelPowderPrep] waiting for prearm completedLayerK={completedLayerK}");
                pendingThread.Join(15000);
            }
        ...
        private bool IsMeteorPrecisionPiSetHomeMotionBranch()
        {
            return EnvEquals("METEOR_AUTO_PRINT_MOTION_BRANCH", MeteorAutoPrintMotionBranchPrecision)
        ...
        {
            DateTime deadline = DateTime.UtcNow.AddMilliseconds(timeoutMs);
            double previousMm = GetCurrentPos(8);
            int stableCount = 0;
            while (DateTime.UtcNow < deadline)
            {
                Thread.Sleep(20);
                double currentMm = GetCurrentPos(8);
                int status = 0;
                motionMap.GetAxisStatus(8, out status);
                stableCount = Math.Abs(currentMm - previousMm) <= 0.005 && (status & 0x400) == 0
                    ? stableCount + 1 : 0;
                if (stableCount >= 3)
                    return true;
            ...
            Log4Net.Info($"[PowderReturnEarlyPrep] axis8 return-rise settle timeout current={GetCurrentPos(8):F3}mm");
            return false;
        }
            ...
            double returnVelocity1, double returnVelocity2, double yJetOffWidth)
        {
            ...
            double pass0TargetY = passStartBaseY - yJetOffWidth;
            if (!WaitInkCarYAxisReach(pass0TargetY, InkCarPassYReachToleranceMm, 15000))
            {
                _meteorMotionAbortLayerK = meteorMotionLayerK;
            ...
            {
                _meteorMotionAbortLayerK = meteorMotionLayerK;
                Log4Net.Info("AutoPrintThread5[physical_home_fast]: axis8 return-rise did not settle before Pass0 scan; abort");
                return false;
            }
            ...
            double returnVelocity1, double yJetOffWidth)
        {
            ...
            if (pass1PrepackSendSplit)
            {
                ...
                {
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast][early_prep]: Pass1 swath prepack wait timeout; abort");
                    return false;
                }
                ...
                Log4Net.Info("[PowderReturnEarlyPrep] Pass1 prepack ready; X scan started and Meteor send released");
                if (!MeteorPrintEngine.WaitPassSwathMeteorReady(1, 10000))
                {
                    motionMap.StopMotion(1, true);
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast][early_prep]: Pass1 swath send wait timeout; X stopped and abort");
                    return false;
                }
                MeteorPrintEngine.TriggerPassForPureMeteorSchedule((uint)(k_nCurrentLayer + 1), 1, "pass1-afterPrepackSend-physical-fast");
            }
            else
            {
                if (!MeteorPrintEngine.WaitPassSwathMeteorReady(1, 10000))
                {
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast]: Pass1 swath wait timeout; abort");
                    return false;
                }
            ...
            MeteorPrintEngine.LogDualCoordSnapshot("Pass1AtScanHighEndPhysicalFast", GetCurrentPos(1));
            ...
            ref SendMessageToCamera toCamera, int recordLayerIndex, int pauseFlag, int notGoCleanStationFlag)
        {
            ...
            if (pass2PrepackSendSplit)
            {
                ...
                {
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast][early_prep]: Pass2 swath prepack wait timeout; abort");
                    return false;
                }
                BackToStation((float)InkCarScanLowXMm, (float)returnVelocity1, false, false, 1);
                MeteorPrintEngine.SignalPassMotionStartedForMeteorSend(2);
                Log4Net.Info("[PowderReturnEarlyPrep] Pass2 prepack ready; X scan started and Meteor send released");
                if (!MeteorPrintEngine.WaitPassSwathMeteorReady(2, 10000))
                {
                    motionMap.StopMotion(1, true);
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast][early_prep]: Pass2 swath send wait timeout; X stopped and abort");
                    return false;
                }
                MeteorPrintEngine.TriggerPassForPureMeteorSchedule((uint)(k_nCurrentLayer + 1), 2, "pass2-afterPrepackSend-physical-fast");
            }
            else
            {
                if (!MeteorPrintEngine.WaitPassSwathMeteorReady(2, 10000))
            ...
            if (notGoCleanStationFlag != 1)
            {
                if (pauseFlag != 1)
                {
                    ...
                    Thread cleanReturnEndJobThread = new Thread(() =>
                    {
                        try
                        {
                            if (MeteorPrintEngine.TryCompleteDeferredEndJob("Pass2AtScanLowEndBeforeCleanReturnPhysicalFast"))
                                MeteorPrintEngine.LogPccMotionSnapshot("Pass2AtScanLowEndAfterDeferredEndJobPhysicalFast");
                        }
                        catch (Exception ex)
                        {
                            Log4Net.Info($"AutoPrintThread5[physical_home_fast]: async EndJob during clean return exception: {ex.Message}");
                        }
                        finally
                        {
                            cleanReturnEndJobDone = true;
                        }
                    });
                    cleanReturnEndJobThread.IsBackground = true;
                    cleanReturnEndJobThread.Start();
                    if (!ReturnInkCarToCleanWaitStationAfterPass2((float)returnVelocity2, xReturnAlreadyStarted: true))
                    {
                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                        motionMap.StopMotion(1, true);
                        motionMap.StopMotion(2, true);
                    ...
                    if (!cleanReturnEndJobThread.Join(10000))
                        Log4Net.Info("AutoPrintThread5[physical_home_fast]: async EndJob during clean return still running after 10000ms");
                    Log4Net.Info($"AutoPrintThread5[physical_home_fast]: clean return EndJob overlap completed={cleanReturnEndJobDone}");
                }
                else
                {
                    BackToStation((float)InkCarScanHighXMm, (float)returnVelocity1, false, false, 1);
                    ...
                    {
                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                        Log4Net.Info("AutoPrintThread5[physical_home_fast]: Pass2 X did not reach scan high end before pause; abort");
                        return false;
                    }
                ...
                if (toCamera != null && toCamera.k_MonitorPrintParam != null && toCamera.k_MonitorPrintParam.m_anJettingBinderBedMonitorFlags[6])
                    toCamera.SendMessageFromSharedMemory(false, recordLayerIndex, 7);
                bool flashRet = MeteorPrintEngine.SetFlash(false);
                Log4Net.Info($"开启闪喷操作（physical_home_fast Pass2末）：IDP_FlashPrtCtl：返回值{{{flashRet}}}");
            }
            else
            {
                BackToStation((float)InkCarScanHighXMm, (float)returnVelocity1, false, false, 1);
                ...
                {
                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                    Log4Net.Info("AutoPrintThread5[physical_home_fast]: Pass2 X did not reach scan high end before no-clean exit; abort");
                    return false;
                }
                MeteorPrintEngine.LogDualCoordSnapshot("Pass2AtScanHighEndPhysicalFast", GetCurrentPos(1));
                if (MeteorPrintEngine.TryCompleteDeferredEndJob("Pass2AfterScanHighEndPhysicalFast"))
            ...
            Log4Net.Info("[MeteorMotionBranch] physical_home_fast Pass2 end");
            return true;
        }
        ...
        public void AutoPrintThread5(int Command, int PassIndex, float m_MovSpeed, float m_BackCleanMovSpeed, ref SendMessageToCamera toCamera, int RecordLayerIndex, int RecordProcessIndex, int PauseFlag, double YJetOffWidth, int NotGoCleanStationFlag,double YJetBaseOffWidth)//20220513新建:自动喷墨运动动作
        {
            ...
            {
                Log4Net.Info($"AutoPrintThread5: skip PassIndex={PassIndex} because Meteor motion aborted layerK={meteorMotionLayerK}");
                return;
            }
            ...
            else if (Command == 1)//小幅面偏移：单层 InkCarPassCountPerLayer PASS，Y 步距 InkCarPassPitchYMm
            {
                ...
                double passPitchY = InkCarPassPitchYMm;/*每 PASS Y 向步距 mm，含双喷头安装补偿*/
                const double passStartBaseY = InkCarPassStartBaseYMm;/*首条带基准：与 PrintRasterConfig 同源，0=平台下边*/
                double ReturnVelocity1 = m_szMovSpeed/*20*/;//喷墨移动速度
                ReturnVelocity1 = m_MovSpeed;
                double ReturnVelocity2 = m_BackCleanMovSpeed;//20230404新增：
                {
...
#region 监控指令：喷墨拍摄位点2
                            ...
                            if (IsMeteorPrecisionPiSetHomeMotionBranch())
                            {
                                // 精扫：每层先在 Home 点停稳并 PiSetHome+STARTJOB，再到 525mm 连续预灌全部 swath。
                                ...
                                if (!MeteorPrintEngine.WaitPass0PreheatStartJobDoneIfEnabled(10000))
                                {
                                    ...
                                    try
                                    {
                                        EnsureInkCarAtCleanWaitStation((float)ReturnVelocity2, "AutoPrintThread5:StartJobAbort");
                                        Log4Net.Info($"AutoPrintThread5: ink car returned to clean wait station after StartJob abort x={GetCurrentPos(1):F3} y={GetCurrentPos(2):F3}");
                                    }
                                    catch (Exception ex)
                                    {
                                        Log4Net.Info($"AutoPrintThread5: EnsureInkCarAtCleanWaitStation after StartJob abort failed: {ex.Message}");
                                    }
                                    return;
                                }
                                if (IsInkCarFastOfficialQueuedMotionEnabled())
                                {
                                    BackToStation((float)InkCarScanApproachXMm, (float)ReturnVelocity2, false, false, 1);
                                    MeteorPrintEngine.SignalPrintThreadLayerPass0ReadyForMeteorSubmit(k_nCurrentLayer + 1);
                                    if (!MeteorPrintEngine.WaitPass0FirstSwathMeteorReady(30000))
                                    {
                                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                                        Log4Net.Info("AutoPrintThread5: fast official-queued all-swath queue wait timeout; abort pass0");
                                        return;
                                    }
                                    Log4Net.Info("[InkCarTiming] FastOfficialQueued Pass0 queue ready at approach");
                                }
                                else if (IsInkCarConservativeTimingEnabled())
                                {
                                    MeteorPrintEngine.SignalPrintThreadLayerPass0ReadyForMeteorSubmit(k_nCurrentLayer + 1);
                            ...
                            {
                                // 默认稳妥序：750→525 过 Home，无每层 PiSetHome/663.5 停靠
                                if (IsInkCarConservativeTimingEnabled())
                                {
                                    MeteorPrintEngine.SignalPrintThreadLayerPass0ReadyForMeteorSubmit(k_nCurrentLayer + 1);
                                    if (!MoveInkCarXToApproachHoldAndWaitPass0Swath((float)ReturnVelocity2, 10000))
                                    {
                                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                                        Log4Net.Info("AutoPrintThread5[default]: approach hold parallel (525+swath) failed; abort pass0");
                                        return;
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                                        Log4Net.Info("AutoPrintThread5[default]: X axis did not settle at 525mm before swath queue; abort pass0");
                                        return;
                                    }
                                    MeteorPrintEngine.SignalPrintThreadLayerPass0PreheatReady(k_nCurrentLayer + 1, "pass0-approach525-default");
                                    MeteorPrintEngine.SignalPrintThreadLayerPass0ReadyForMeteorSubmit(k_nCurrentLayer + 1);
                                    if (!MeteorPrintEngine.WaitPass0FirstSwathMeteorReady(10000))
                                    {
                                        _meteorMotionAbortLayerK = meteorMotionLayerK;
                                        Log4Net.Info("AutoPrintThread5[default]: Pass0 swath wait timeout; abort scan motion");
                                        return;
                                    }
                                }
                            }
                            ...
                            if (!IsInkCarFastOfficialQueuedMotionEnabled() && !WaitInkCarXAxisAtScanEndpoint(InkCarScanLowXMm))
                            {
                                _meteorMotionAbortLayerK = meteorMotionLayerK;
                            ...
                            if (IsInkCarFastOfficialQueuedMotionEnabled() || IsInkCarConservativeTimingEnabled())
                                BackToStation(passStartBaseY + passPitchY - YJetOffWidth, (float)ReturnVelocity1, true, false, 1);//快速/稳妥：Y 异步，Pass1 入口再 Reach
                            else
...
#region 监控指令：喷墨拍摄位点3
                            ...
                            {
                                double pass1TargetY = passStartBaseY + passPitchY - YJetOffWidth;
                                if (!IsInkCarFastOfficialQueuedMotionEnabled() && IsInkCarConservativeTimingEnabled() && !WaitInkCarYAxisReach(pass1TargetY, 1.0, 15000))
                                {
                                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                            ...
                            {
                                BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, false, 1);
                                Log4Net.Info("[InkCarTiming] FastOfficialQueued Pass1 async scan depart");
                            }
                            else if (MeteorPrintEngine.IsAllFwdDiagnosticModeEnabled())
                            {
...
#region 监控指令：喷墨拍摄位点3
                        ...
                        case 2://第3 PASS：swath 先入 PCC，再启动 485→15 扫程；pass 切换靠 natural PD（不用 ForcePD）
                            ...
                            {
                                double pass2TargetY = passStartBaseY + 2 * passPitchY - YJetOffWidth;
                                if (!IsInkCarFastOfficialQueuedMotionEnabled() && IsInkCarConservativeTimingEnabled() && !WaitInkCarYAxisReach(pass2TargetY, 1.0, 15000))
                                {
                                    _meteorMotionAbortLayerK = meteorMotionLayerK;
                            ...
                            {
                                BackToStation((float)InkCarScanLowXMm, (float)ReturnVelocity1, false, false, 1);
                                Log4Net.Info("[InkCarTiming] FastOfficialQueued Pass2 async scan depart");
                            }
                            else if (MeteorPrintEngine.IsAllFwdDiagnosticModeEnabled())
                            {
...
#region 监控指令：喷墨拍摄位点4
...
#endregion
                            BackToStation((float)InkCarScanHighXMm, (float)ReturnVelocity1, false, (IsInkCarFastOfficialQueuedMotionEnabled() || IsInkCarConservativeTimingEnabled()) ? false : true, 1);//15→525 收口
                            if (!IsInkCarFastOfficialQueuedMotionEnabled())
                            {
                                if (IsInkCarConservativeTimingEnabled())
                                {
                            ...
                            if (MeteorPrintEngine.TryCompleteDeferredEndJob("Pass2AfterScanHighEnd"))
                                MeteorPrintEngine.LogPccMotionSnapshot("Pass2AfterScanHighEndAfterDeferredEndJob");
...
#region 监控指令：喷墨拍摄位点7
        ...
        /// <remarks>
        /// 阶段B路由表：Command1→AutoCleanThread2；Command2→NewAutoSupplyPowderThread2*；Command3→AutoCureThread；
        /// [2026-06-01] 自动打印墨车运动量产主路径：Command6→AutoPrintThread5（TwoPassPrintPerThreeTimes / 3PASS+Meteor）；
        /// Command4→AutoPrintThread2、Command5→AutoPrintThread3 为其它编译配置/诊断路径。
        /// </remarks>
        ...
        /// [2026-06-01] 与量产一致：依次 AutoPrintThread5（等价于工业控制 EquipmentMotionLogic3 Command=6），
        /// 并补执行 PassIndex=3 层后/观察站收尾。
        /// </summary>
...
#pragma warning restore CS0618
        ...
        private const string MeteorPhysicalHomeFastImageMaxWidthPx = "7244";
        ...
        private void ApplyMeteorPiSetHomePhysicalHomeFixedXStartMode()
        {
            ...
            SetProcessEnv("METEOR_INKCAR_CONSERVATIVE_TIMING", "1");
            ...
            SetProcessEnv("METEOR_MIN_MS_AFTER_STARTJOB_FOR_SETHOME", "400");
        ...
        private bool IsMeteorPhysicalHomeFastPrintUiMode()
        {
            return IsMeteorPhysicalHomeFastPrintMotionBranch();
        }

        private void UpdateMeteorPhysicalHomeFastPrintButtonText()
        {
            try
            {
                if (buttonMeteorPhysicalHomeFastPrint == null)
                    return;
                bool active = IsMeteorPhysicalHomeFastPrintUiMode();
                buttonMeteorPhysicalHomeFastPrint.Text = active
                    ? "物理Home快速\r\n打印已设"
                    : "切到物理Home\r\n快速打印";
                buttonMeteorPhysicalHomeFastPrint.BackColor = active ? Color.LightSkyBlue : Color.AliceBlue;
                buttonMeteorPhysicalHomeFastPrint.ForeColor = active ? Color.DarkBlue : Color.DarkSlateBlue;
            }
            catch { }
        }
        ...
        {
            return IsMeteorPhysicalHomeFastPrintMotionBranch()
                && EnvEquals(PowderReturnEarlyPass0PrepModeEnv, "1");
        }

        private void UpdatePowderReturnEarlyPass0PrepButtonText()
        {
            try
            {
                if (buttonPowderReturnEarlyPass0Prep == null)
                    return;
                bool active = IsPowderReturnEarlyPass0PrepUiMode();
                buttonPowderReturnEarlyPass0Prep.Text = active
                    ? "粉车440后并行\r\nPass0 已启用"
                    : "启用粉车440后\r\n并行 Pass0";
                buttonPowderReturnEarlyPass0Prep.BackColor = active ? Color.LightSalmon : Color.MistyRose;
                buttonPowderReturnEarlyPass0Prep.ForeColor = active ? Color.DarkRed : Color.Maroon;
            }
            catch { }
        }
        ...
        private void buttonMeteorPiSetHomeFixedXStart_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("已切换为 PiSetHome 精扫分支。\r\n每层 663.5mm Home + PiSetHome + OfficialQueued 预灌；运动走精扫专用序。\r\n与「物理 Home 快速打印」互斥，对下一次打印生效。");
        }

        private void buttonMeteorPhysicalHomeFastPrint_Click(object sender, EventArgs e)
        {
            if (HasAutoMotionThreadRunning())
            {
                MessageBox.Show("当前有自动打印/清洗/固化线程运行，请停止后再切换 Meteor 扫描链路。");
                Log4Net.Info("Meteor 物理Home快速打印链路切换被拒绝：自动运动线程正在运行。");
                return;
            }
        ...
        private void buttonPowderReturnEarlyPass0Prep_Click(object sender, EventArgs e)
        {
            if (HasAutoMotionThreadRunning())
            {
                MessageBox.Show("当前有自动打印/清洗/固化线程运行，请停止后再切换层间快速预备模式。");
                Log4Net.Info("粉车回程提前 Pass0 预备模式切换被拒绝：自动运动线程正在运行。");
                return;
            }

            if (!IsMeteorPhysicalHomeFastPrintMotionBranch())
            {
                MessageBox.Show("请先切换为「物理 Home 快速打印」分支，再启用粉车回程提前 Pass0 预备。");
                return;
            }
        ...
        public void InitKidFormWithAutoPrintParamInTest()//20201020新建：数据绑定自动固化清洗-自动进给铺粉-自动铺粉续打
        {
            ...
            textBox49.DataBindings.Add("Text", k_RYSYSParamAutoPrintParamInTest, "Pass0EffectiveYTrimMm", true /*false*/, DataSourceUpdateMode.OnPropertyChanged);
            textBox50.DataBindings.Add("Text", k_RYSYSParamAutoPrintParamInTest, "Pass2EffectiveYTrimMm", true /*false*/, DataSourceUpdateMode.OnPropertyChanged);
        ...
        public class AutoPrintParamInTest : INotifyPropertyChanged, ICloneable//C#中，通知类的属性值已经更改，可以避免大量的通用事件的使用；其中关键是属性的理解及和lambda表达式的使用方法
        {
            ...
            public int m_nEnableTravelDispenseAxis3 = 0;
            ...
            public double m_dPass0EffectiveYTrimMm = 0;
            /// <summary>Pass2 平台内有效 Y 上沿补偿(mm)：正值裁减上侧/platform 外喷嘴对应图形行。</summary>
            public double m_dPass2EffectiveYTrimMm = 0;
            ...
            /// <summary>同步铺粉去程：铺粉车轴7绝对坐标(mm)，IR/UV 开始出光位置；默认100，允许0（粉床下沿/Home附近）。</summary>
            public double m_dSyncCureOpenPosMm = 100;
            ...
            public double Pass0EffectiveYTrimMm
            {
                get { return this.m_dPass0EffectiveYTrimMm; }
                set
                {
                    if (value != this.m_dPass0EffectiveYTrimMm && value >= -50 && value <= 50)
                    {
                        this.m_dPass0EffectiveYTrimMm = value; NotifyPropertyChanged();
                    }
                }
            }

            public double Pass2EffectiveYTrimMm
            {
                get { return this.m_dPass2EffectiveYTrimMm; }
                set
                {
                    if (value != this.m_dPass2EffectiveYTrimMm && value >= -50 && value <= 50)
                    {
                        this.m_dPass2EffectiveYTrimMm = value; NotifyPropertyChanged();
                    }
                }
            }
            ...
            public double SyncCureOpenPosMm
            {
                ...
                set
                {
                    const double minOpenMm = 0;
                    double v = Math.Max(minOpenMm, Math.Min(value, 1017.0 - 5));
                    if (v >= m_dSyncCureClosePosMm - 5)
                        v = Math.Max(minOpenMm, m_dSyncCureClosePosMm - 5);
