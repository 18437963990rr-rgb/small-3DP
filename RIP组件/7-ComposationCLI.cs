...
namespace Composation//统一修改为BinderJetting命名空间
{
    class ComposationCLI
    {
        ...
        public void Composation(ref List<CLI> CliStreams,double innerSafeGap,double borderSafeGap)// 内部安全和外部安全距离默认采取2mm
        {
            ...
            //Initiate the tempX:
            //double tempX = PrintRasterConfig.PlateWidthMm-2* borderSafeGap;//record the spare row length in BASE, and the iniatiate value is 420-2*borderSafeGap
            //double tempY = PrintRasterConfig.PlateHeightMm-2* borderSafeGap;//record the spare colunm length in BASE , and the iniatiate value is 350-2*borderSafeGap
            ...
            {
                if (jobItems[k-1].position.X+ jobItems[k-1].Width + jobItems[k].Width+innerSafeGap<PrintRasterConfig.PlateWidthMm-borderSafeGap)//if a row is not arrange well yet.//Arrange a row:
                {                 
                    ...
                    //jobItems[k].position.Y = PrintRasterConfig.PlateHeightMm- borderSafeGap- tempY;
                ...
                else//when a row is arrange well.//Switch Rows: 
                {
                    ...
                    //tempX= PrintRasterConfig.PlateWidthMm - 2 * borderSafeGap- jobItems[k].Width - innerSafeGap;
                    //tempY = PrintRasterConfig.PlateHeightMm - borderSafeGap - tempY - PreviousMaxRowHeight;
